...
export default function NavigationSettings() {
  ...
  return (
    <SettingsLayout
      title="Navigation"
      description="Customize how you navigate through TaskQuest"
    >
      <div className="space-y-8">
        ...
        <div className="space-y-4">
          ...
          <div className="border rounded-lg p-4 bg-muted/30">
            <div
              className={`flex ${
                navPosition === "left" ? "flex-row" : "flex-col"
              } gap-2`}
            >
              {navPosition === "left" && (
                <div className="flex flex-col gap-2 border-r pr-4">
                  {renderNavItems(navStyle)}
                </div>
              )}
              {navPosition === "top" && (
                <div className="flex flex-row gap-4 border-b pb-4 mb-4">
                  {renderNavItems(navStyle)}
                </div>
              )}
              ...
              {navPosition === "bottom" && (
                <div className="flex flex-row justify-around gap-4 border-t pt-4 mt-4">
                  {renderNavItems(navStyle)}
                </div>
              )}
            </div>
...
function renderNavItems(style: string) {
  ...
  return items.map((item, index) => (
    ...
    >
      ...
      {(style === "icons-and-text" || style === "text-only") && (
