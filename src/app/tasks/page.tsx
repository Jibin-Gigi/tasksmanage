...
export default function TasksPage() {
  ...
  const toggleQuestExpansion = (questId: string) => { // Move function here
        ...
        <div className="min-h-screen bg-[#030014] text-white p-8 md:pl-20">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="mb-8">
              ...
              <p className="text-violet-300/90 mt-2 text-lg">View and verify your tasks</p>
            </div>
            ...
            {/* Dailies Section */}
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-violet-100 mb-4">Daily Tasks</h2>
                ...
                {filteredDailies.map(daily => (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <button
                        onClick={() => handleVerifyTask(daily.id, daily.category)}
                      ...
                      <div className="flex-1">
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            ...
                            <p className="text-violet-300/90 mt-2">{daily.description}</p>
                          </div>
            ...
            {/* Quests Section */}
            <div>
              <h2 className="text-2xl font-semibold text-violet-100 mb-4">Quests</h2>
              <div className="space-y-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            ...
                            <p className="text-violet-300/90 mt-2">{quest.description}</p>
                          </div>
                          ...
                          <div className="mt-6 pl-4 border-l-2 border-violet-500/30 space-y-4">
                            {quest.selected_tasks.map((task, index) => (
                              <div key={task.id} className="flex items-start gap-4">
                                <button
                                  onClick={() => handleVerifyTask(quest.id, `quest-task-${index}`)}
                                  className="mt-1 p-2 rounded-full bg-violet-950/80 text-violet-400 hover:bg-violet-500/20 transition-all duration-300"
                                >
                                  <Check className="h-4 w-4" />
                                </button>
                        ...
                        <div className="mt-4">
                          <p className="text-sm text-violet-300/90">
                            {quest.selected_tasks.filter(t => t.completed).length} of {quest.selected_tasks.length} tasks completed
                          </p>
                        </div>
                      </div>
                    </div>
