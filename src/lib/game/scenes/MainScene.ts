...
import { Fighter } from "@/lib/game/sprites/Fighter";
...
export class MainScene extends Scene {
  ...
  preload() {
    ...
    this.load.spritesheet(
      "samuraiMack-takeHit",
      "/assets/samuraiMack/Take Hit.png",
      {
        frameWidth: 200,
        frameHeight: 200,
      }
  ...
  create() {
    ...
    this.physics.add.collider(
      ...
      undefined,
      (object1, object2) => {
        const playerBody = (object1 as unknown as Fighter)
          .body as Phaser.Physics.Arcade.Body;
        const enemyBody = (object2 as unknown as Fighter)
          .body as Phaser.Physics.Arcade.Body;
    ...
    this.physics.add.overlap(this.player, this.enemy, () => {
      if (this.player.isAttacking()) {
        ...
        this.enemyHealthBar.width =
          (this.enemy.getCurrentHealth() / this.enemy.getMaxHealth()) * 400;
      }
    });
    ...
    this.physics.add.overlap(this.enemy, this.player, () => {
      if (this.enemy.isAttacking()) {
        ...
        this.playerHealthBar.width =
          (this.player.getCurrentHealth() / this.player.getMaxHealth()) * 400;
      }
    });
  ...
  private createUI() {
    ...
    this.enemyName = this.add
      .text(width - edgePadding, 15, "LIDIYA (BOSS)", {
        fontSize: "16px",
        color: "#fff",
        fontFamily: '"Press Start 2P"',
      })
