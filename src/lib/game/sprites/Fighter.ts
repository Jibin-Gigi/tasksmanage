...
export class Fighter extends Physics.Arcade.Sprite {
  ...
  private createAnimations() {
    ...
    this.anims.create({
      key: "idle",
      frames: this.anims.generateFrameNumbers(`${characterPrefix}-idle`, {
        start: 0,
        end: 7,
      }),
      frameRate: 10,
    ...
    this.anims.create({
      key: "run",
      frames: this.anims.generateFrameNumbers(`${characterPrefix}-run`, {
        start: 0,
        end: 7,
      }),
      frameRate: 10,
    ...
    this.anims.create({
      key: "jump",
      frames: this.anims.generateFrameNumbers(`${characterPrefix}-jump`, {
        start: 0,
        end: 1,
      }),
      frameRate: 10,
    ...
    this.anims.create({
      key: "fall",
      frames: this.anims.generateFrameNumbers(`${characterPrefix}-fall`, {
        start: 0,
        end: 1,
      }),
      frameRate: 10,
    ...
    this.anims.create({
      key: "attack1",
      frames: this.anims.generateFrameNumbers(`${characterPrefix}-attack1`, {
        start: 0,
        end: 5,
      }),
      frameRate: 15,
    ...
    this.anims.create({
      key: "attack2",
      frames: this.anims.generateFrameNumbers(`${characterPrefix}-attack2`, {
        start: 0,
        end: 5,
      }),
      frameRate: 15,
    ...
    this.anims.create({
      key: "takeHit",
      frames: this.anims.generateFrameNumbers(`${characterPrefix}-takeHit`, {
        start: 0,
        end: 3,
      }),
      frameRate: 10,
    ...
    this.anims.create({
      key: "death",
      frames: this.anims.generateFrameNumbers(`${characterPrefix}-death`, {
        start: 0,
        end: 5,
      }),
      frameRate: 10,
  ...
  private calculateDamage(): number {
    ...
    const baseDamage =
      this.BASE_WEAPON_DAMAGE + this.stats.strength * this.STRENGTH_MULTIPLIER;

    // Critical hits are based on agility
    const isCritical =
      Math.random() < (this.stats.agility * this.CRIT_MULTIPLIER) / 100;
  ...
  private canDodge(): boolean {
    ...
    const dodgeChance = Math.min(
      0.5,
      (this.stats.agility * this.DODGE_MULTIPLIER) / 100
    );
  ...
  takeHit(incomingDamage: number = 20): boolean {
    ...
    const finalDamage = Math.max(
      1,
      Math.round(incomingDamage - damageReduction)
    ); // Minimum 1 damage
  ...
  private updateAI() {
    ...
    const playerInactive =
      currentTime - this.lastPlayerAttackTime > this.aiConfig.forceAttackDelay;
    if (playerInactive) {
      ...
      if (
        distanceToTarget > this.aiConfig.attackRange * 2 &&
    ...
    if (
      distanceToTarget <= this.aiConfig.attackRange &&
      !this._isAttacking &&
      currentTime - this.aiConfig.lastAttackTime >
        this.aiConfig.attackCooldown &&
      ...
      const speedMultiplier =
        distanceToTarget < this.aiConfig.attackRange * 1.5 ? 0.5 : 0.8;
  ...
  private calculateMoveSpeed(): number {
    return (
      this.BASE_MOVE_SPEED + this.stats.agility * this.AGILITY_SPEED_MULTIPLIER
    );
  }
